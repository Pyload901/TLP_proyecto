...
static void translator_init(Translator *tr) {
    ...
    tr->temp_top = VM_NUM_REGISTERS; /* temps allocated from the top (exclusive) */
    tr->array_count = 0;
    tr->heap_top = 0;
    tr->function_count = 0;
...
static uint8_t alloc_temp(Translator *tr) {
    if (tr->temp_top <= tr->next_var_reg) {
        fprintf(stderr, "translator debug: alloc_temp failed (next=%u temp_top=%u)\n",
                (unsigned) tr->next_var_reg, (unsigned) tr->temp_top);
...
static void release_temp(Translator *tr, uint8_t reg) {
    if (reg >= tr->temp_top && reg < VM_NUM_REGISTERS) {
        tr->temp_top++;
    }
}
...
bool translate_program(Node *root, const char *output_path) {
    ...
    if (ok && !tr.failed) {
        ...
        if (!out) {
            ...
            fprintf(out, "# TinyVM instruction listing generated by translator\n");
            fprintf(out, "# format: <index> <mnemonic> <arg1> <arg2>\n");
            size_t count = tr.code.size / 3;
